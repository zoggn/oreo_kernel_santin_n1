...
static SPI_SPEED trans_spi_speed(unsigned int speed) {
  ...
  if (speed >= 4000000) {
    spi_speed = SPEED_8MHZ; // Overclocked a bit 4Mhz to 8Mhz 
  }
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
  ...
  case GF_IOC_SENDKEY:
    ...
    input_report_key(gf_dev->input, KEY_BACK, gf_key.value);
 ...
 ***********************************************************/
static int gf_spi_write_bytes(struct gf_dev *gf_dev, u16 addr, u32 data_len,
                              u8 *tx_buf) {
  ...
  if ((package_num > 0) && (reminder != 0)) {
    ...
    memcpy(reminder_buf + GF_WDATA_OFFSET,
           tx_buf + 2 * GF_WDATA_OFFSET + data_len - reminder, reminder);
    gf_print("gf:w-reminder:0x%x-0x%x,0x%x", reminder_buf[GF_WDATA_OFFSET],
             reminder_buf[GF_WDATA_OFFSET + 1],
  ...
  gf_print(
      "gf:write twice = %d. data_len = %d, package_num = %d, reminder = %d\n",
...
int gf_spi_read_bytes(struct gf_dev *gf_dev, u16 addr, u32 data_len,
  ...
  one_more_buff = kzalloc(28000, GFP_KERNEL);
  ...
  // gf_debug(SPI_DEBUG,"gf:read twice = %d, data_len = %d, package_num = %d,
  // reminder = %d\n",(int)twice, (int)data_len, (int)package_num,
  // (int)reminder);
...
static int gf_spi_transfer(struct gf_dev *gf_dev, unsigned long arg) {
  ...
  if (copy_from_user(&ioc, (struct gf_spi_transfer *)arg,
                     sizeof(struct gf_spi_transfer))) {
  ...
  if (ioc.cmd == GF_R) {
    ...
    if (copy_to_user((void __user *)ioc.buf,
                     (void *)(gf_dev->gBuffer + GF_RDATA_OFFSET), ioc.len)) {
    ...
    if (copy_from_user(gf_dev->gBuffer + GF_WDATA_OFFSET, (void *)ioc.buf,
                       ioc.len)) {
...
static int gf_probe(struct spi_device *spi) {
  ...
  gf_dev->gBuffer =
      (unsigned char *)__get_free_pages(GFP_KERNEL, get_order(BUF_SIZE));
  ...
  status = __register_chrdev(156, 0, 256, CHRD_DRIVER_NAME,
                             &gf_fops); // register char devices.
  if (status < 0) {
  ...
  if (IS_ERR(gf_class)) {
    __unregister_chrdev(156, 0, 256, gf_driver.driver.name);
  ...
  if (status == 0) {
    ...
    ret =
        request_threaded_irq(gf_dev->spi->irq, NULL, gf_irq,
...
error:
  ...
  int get_frame_data(struct gf_dev * gf_dev, unsigned char *pFrame, int len) {
    ...
    do {
      ...
      if (fifo_len_tmp >= fifo_len_next) {
        ...
        fifo_len_next = (fifo_len_next == GF_READ_BUFF_SIZE1)
                            ? GF_READ_BUFF_SIZE2
                            : GF_READ_BUFF_SIZE1;
        if (total_len + fifo_len <= GF_SPI_DATA_LEN) {
          //                gf_secspi_read_fifo(gf_secdev, GF_DATA_ADDR,
          //                fifo_len
          //                - 4,
          //                        buf + total_len);
  ...
  static int restructure_raw_data(unsigned char *pFrame, unsigned char *pFpdata,
                                  int total_len) {
    int i = 0, j = 0;
    unsigned short id = 0;
    unsigned short calc_crc = 0;
    unsigned char crc_buf[GF_ROW_DATA_SIZE] = {0};
    gx_row_t row = {0};
    ...
    for (i = 0; i < SENSOR_ROW; i++) {
      ...
      if (calc_crc == row.crc) {
        ...
        memcpy(pFpdata + row.id * GF_ROW_DATA_SIZE, (unsigned char *)row.raw,
               GF_ROW_DATA_SIZE);
      } else {
        gf_print("Row %d crc error, calc_crc=%x, read_crc=%x", row.id, calc_crc,
                 row.crc);
        return -1;
      }
    }
  ...
  int gf_read_image(struct gf_dev * gf_dev, unsigned char *pImage, int len) {
    ...
    buf = (unsigned char *)__get_free_pages(GFP_KERNEL,
                                            get_order(GF_SPI_DATA_LEN));
    if (buf == NULL) {
      gf_print("[%s]get_free_pages error", __func__);
      return -1;
    }
  ...
  /* We use late_initcall because driver should be registered after rt5081_ldo,
   */
  /* either it gives error about not founding rt5081_ldo node */
